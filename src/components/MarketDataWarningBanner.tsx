...
function MarketDataWarningBanner() {
  const { showBanner, severity, messages } = useMarketDataHealth()
  const { refreshPrices, isRefreshingPrices } = useData()
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <button
          ...
          className="flex-shrink-0 rounded-input border border-border-subtle px-2 py-1 text-[0.55rem] md:text-[0.63rem] text-text-secondary hover:bg-bg-surface-2 disabled:opacity-50"
        >
          {isRefreshingPrices ? 'Retrying…' : 'Retry'}
        </button>
      </div>
    </div>
  )
...
export default MarketDataWarningBanner
